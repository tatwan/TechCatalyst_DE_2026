...
* Make sure to include all your `venv` related python environments that you created. If you used `python -m venv <some env>` list that environment folder inside the `.gitignore`:

> [!NOTE]
> Do **not** add `.gitignore` itself to the ignore list — you want it tracked and committed so the whole team shares the same rules (see Lab B and slide 25).
...
# Ignore my virtual environments 
dev2/
devpy/
MyWork/
Class Work/
