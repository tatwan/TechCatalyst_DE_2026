...
> [!IMPORTANT]
> **Budget note:** this lab needs **no Codespace** — you will edit right in the GitHub web editor. A profile page is tiny; spinning up a Codespace would just burn the free core-hours you learned to protect this morning. (If you *want* the Git practice, an optional "do it the Git way" path is at the end.)
...
Prefer to practice today's skills? You can clone the special repo into a Codespace and use the full cycle:

```bash
# from the repo page: Code ▸ Codespaces ▸ Create codespace on main
# then, in the Codespace terminal:
# edit README.md in the editor, then:
git add README.md
git commit -m "Write my profile README"
git push
```

Same result — your profile updates on push. Just remember to **stop the Codespace** when you finish so you do not burn core-hours on a one-file edit.
