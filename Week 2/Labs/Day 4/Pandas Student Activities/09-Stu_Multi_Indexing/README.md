...
2. In the `read_csv` function, set the index to equal `Date` series. Enable read_csv's `parse_dates` parameter (pandas infers the datetime format automatically; the old `infer_datetime_format` parameter was removed in pandas 3.0).
